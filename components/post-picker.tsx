...
export default function PostPicker({
  ...
  useEffect(() => {
    ...
    fetch(`/api/instagram/posts${params.size ? `?${params}` : ""}`)
      ...
      .catch(() => {
        if (!cancelled && !cached.data) setError("Não foi possível carregar as publicações");
      })
      .finally(() => {
  ...
  if (error) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-xs text-zinc-500 mt-1">Conecte primeiro sua conta do Instagram</p>
      </div>
    );
  }
      ...
      {visible.length === 0 ? (
        ...
        <>
          ...
          <button
            ...
            title={isUsed ? `Já utilizada pela campanha "${usedByName}"` : undefined}
            className={`
